-- Replace award_monthly_winners with unambiguous return signature
-- Drops existing functions with conflicting signatures, then recreates.

-- 1) Drop dependent wrapper first (if exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'award_monthly_winner'
      AND p.proargtypes = (SELECT ARRAY[regtypein('text'::cstring)]::oidvector)
  ) THEN
    DROP FUNCTION award_monthly_winner(text);
  END IF;
EXCEPTION WHEN OTHERS THEN
  -- ignore
END$$;

-- 2) Drop the main function with old signature
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'award_monthly_winners'
      AND p.proargtypes = (SELECT ARRAY[regtypein('text'::cstring), regtypein('integer'::cstring)]::oidvector)
  ) THEN
    DROP FUNCTION award_monthly_winners(text, integer);
  END IF;
EXCEPTION WHEN OTHERS THEN
  -- ignore
END$$;

-- 3) Recreate the main function with month_text to avoid ambiguity
...
-- 4) Recreate wrapper that maps month_text -> month for legacy callers
