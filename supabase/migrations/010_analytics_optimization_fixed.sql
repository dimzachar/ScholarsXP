-- Analytics Optimization Migration (PostgreSQL Compatible)
...
SELECT 
  ...
  COUNT(DISTINCT CASE WHEN cf.status = 'PENDING' THEN cf.id END) as pending_flags,
  -- Performance metrics (PostgreSQL compatible)
  ROUND(
    (CASE 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_last_active_at ON "User"("lastActiveAt") WHERE "lastActiveAt" IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_submission_created_at_status ON "Submission"("createdAt", "status");
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_peer_review_created_at ON "PeerReview"("createdAt");
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xp_transaction_created_at_amount ON "XpTransaction"("createdAt", "amount") WHERE "amount" > 0;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_achievement_earned_at ON "UserAchievement"("earnedAt");
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_content_flag_status ON "ContentFlag"("status") WHERE "status" = 'PENDING';

-- Create composite indexes for better join performance
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_submission_user_created_status ON "Submission"("userId", "createdAt", "status");
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_peer_review_reviewer_created ON "PeerReview"("reviewerId", "createdAt");
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xp_transaction_user_created_amount ON "XpTransaction"("userId", "createdAt", "amount");
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_achievement_user_earned ON "UserAchievement"("userId", "earnedAt");
