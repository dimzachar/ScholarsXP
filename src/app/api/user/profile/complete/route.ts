...
async function getOptimizedCompleteProfile(userId: string): Promise<CompleteUserProfileDTO> {
  ...
  return await withQueryCache(
    ...
    async () => {
      ...
      const xpBreakdown = xpTransactions.reduce((acc, tx) => {
        switch (tx.type) {
          case 'SUBMISSION':
            acc.submissions += tx.amount
            break
          case 'PEER_REVIEW':
            acc.reviews += tx.amount
            break
          case 'ACHIEVEMENT':
            acc.achievements += tx.amount
            break
          default:
            acc.other += tx.amount
        }
        return acc
      }, { submissions: 0, reviews: 0, achievements: 0, other: 0 })

      const submissionXp = xpBreakdown.submissions
      const reviewXp = xpBreakdown.reviews
      const achievementXp = xpBreakdown.achievements
      const otherXp = xpBreakdown.other
