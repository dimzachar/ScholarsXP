...
/**
 * Analyze a divergent case using LLM
 */
export async function analyzeDivergentCase(input: CaseAnalysisInput): Promise<CaseAnalysisResult> {
  try {
    ...
    const reviewsText = input.reviews
      .map(r => {
        const meta = [r.category, r.tier ? `Tier ${r.tier}` : null].filter(Boolean).join(', ')
        return `Reviewer ${r.label} (${r.xpScore} XP${meta ? `, ${meta}` : ''}): "${r.comments || 'No comment provided'}"`
      })
      .join('\n')
...
export function analyzeWithoutLLM(input: CaseAnalysisInput): CaseAnalysisResult {
  ...
  if (zeroReviewer && highReviewer) {
    ...
    if (zeroReviewer.comments?.toLowerCase().includes('not available') ||
        zeroReviewer.comments?.toLowerCase().includes('unavailable') ||
        zeroReviewer.comments?.toLowerCase().includes('deleted')) {
